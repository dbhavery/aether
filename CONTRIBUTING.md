# Contributing to Free Companion — Community Edition
...
Companion is an architecture project in an early preview state. That means the
...
1. Read `docs/ARCHITECTURE-V2.md`. This is the architecture reference. If your
   idea conflicts with it, open an issue before writing code.
2. Read `docs/PRODUCT-PLAN.md` for the hard rules and product direction.
3. Read `docs/adr/` to understand how the seven layers relate and why.
4. Skim the most recent `WAVE*_EXECUTION_REPORT_*.md` to see where the project
   actually is today.
5. Check existing issues before proposing anything substantial.
...
- **First-logic slices for L1/L2/L3/L4/L6/L7** — each layer's target surface
  is described in `docs/ARCHITECTURE-V2.md` and the records under `docs/adr/`.
  Pick one, open an issue to claim it, produce a wave report.
...
1. Propose in a PR that touches only `docs/` and the relevant
   `README.md` / spec file.
