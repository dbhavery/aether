...
**Title:**
Show HN: Companion – local-first AI companion architecture with a non-bypassable policy gate (Rust)
...
Companion is an architecture for a long-lived local AI companion. It is preview-grade, MIT-licensed, and doesn't ship a runnable product yet. What it does ship is the spine: a seven-layer Rust workspace where every side-effectful call routes through one non-bypassable policy engine.
...
- **Prepared replies you will get:**
  - *"Why not just use LangChain?"* → Short answer: LangChain is a runtime chain-composer; Companion is a layered architecture with a non-bypassable policy engine. Different thing. Longer answer in the blog post.
